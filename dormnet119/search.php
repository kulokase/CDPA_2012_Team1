		...
		<div style="color:; background:; filter : alpha(opacity=50); opacity : 0.5;">
			...
			<h3>報修詳細資訊</h3>
				...
				<tr>
					...
					<?php 
						echo isset($data['id'])?htmlspecialchars($data['id']):""; 
					?>
					" readonly="readonly"/><br/>
					</td>
					<td><input type="text" name="time" value="
					<?php 
						echo isset($data['timestamp'])?htmlspecialchars($data['timestamp']):""; 
					?>
					" readonly="readonly"/><br/>
					</td>
					<td><input type="text" name="who" value="
					<?php 
						echo isset($data['uname'])?htmlspecialchars($data['uname']):""; 
					?>
					" readonly="readonly"/><br/>
					</td>
					<td><input type="text" name="ip" value="
					<?php 
						echo isset($data['ip'])?htmlspecialchars($data['ip']):""; 
					?>
					" readonly="readonly"/><br/>
					</td>	
					<td><input type="text" name="mac" value="
					<?php 	
						echo isset($data['mac'])?htmlspecialchars($data['mac']):""; 
					?>
					" readonly="readonly"/><br/>
					</td>
					<td><input type="text" name="status" value="
					<?php 
						echo isset($data['status'])?htmlspecialchars($data['status']):""; 
					?>
					" readonly="readonly"/><br />
					</td>
                        	</tr>
				...
				function read_log(){
					...
					$sql_2 = "SELECT * FROM  `process_log` ";
        				...
        				foreach($result as $tmp){
                				...
                				echo htmlspecialchars($tmp['timestamp'])."<br />";
                				echo htmlspecialchars($tmp['manager'])."<br />";
                				echo htmlspecialchars($tmp['log'])."<br /><br/>";
       					}
				}
				read_log ();
			?>
